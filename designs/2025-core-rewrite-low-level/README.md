...
// create the reporter
const reporter = new Reporter({ session })

// create the visitor from the enabled rules
const visitorCreator = new RuleVisitorCreator({ session });
const visitor = await visitorCreator.create({ config, context: fileContext, reporter});
...
The `Reporter` class is responsible for collecting and managing linting messages produced during the analysis of a file. It is constructed with a `Session` instance and provides an interface for storing, retrieving, and formatting messages. The reporter is passed to rule visitors.
